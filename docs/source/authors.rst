Authors
=======

.. image:: _static/dns_logo_horizontal.svg
   :alt: Dynamics and Neural Systems Group
   :target: https://dynamicsandneuralsystems.github.io/
   :width: 400px
   :align: center

``pyhctsa`` is developed and maintained by members of the
`Dynamics and Neural Systems Group <https://dynamicsandneuralsystems.github.io/>`_
at the University of Sydney.

Core team
---------

.. grid:: 2
   :gutter: 3

   .. grid-item-card:: Joshua B. Moore
      :link: https://orcid.org/0000-0002-5237-1087
      :link-type: url

      Author and maintainer.

      :material-regular:`mail;1.2em` joshua.moore@sydney.edu.au

      :material-regular:`fingerprint;1.2em` `ORCID 0000-0002-5237-1087 <https://orcid.org/0000-0002-5237-1087>`_

   .. grid-item-card:: Ben D. Fulcher
      :link: https://orcid.org/0000-0002-3003-4055
      :link-type: url

      Author and maintainer.

      :material-regular:`mail;1.2em` ben.fulcher@sydney.edu.au

      :material-regular:`fingerprint;1.2em` `ORCID 0000-0002-3003-4055 <https://orcid.org/0000-0002-3003-4055>`_

Contributors
------------
``pyhctsa`` is an open, community-driven project. We are grateful to everyone who
has contributed code, documentation, bug reports, and feedback. A full list of
contributors is available on
`GitHub <https://github.com/DynamicsAndNeuralSystems/pyhctsa/graphs/contributors>`_.

Interested in contributing? See the :doc:`development guide <development/index>`
to get started.
