...
def test_medical_module_basic():
    # basic checks on medical module
    data = get_dataset(which="sinusoid")
    config_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "pyhctsa", "Configurations", "medical.yaml")
...
def test_extreme_module_basic():
    # basic checks on extreme events module
    data = get_dataset(which="sinusoid")
    config_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "pyhctsa", "Configurations", "extreme.yaml")
    assert os.path.exists(config_path), f"Config file not found: {config_path}"
    calc = FeatureCalculator(config_path)
    fvec = calc.extract(data)
    # Check that something is returned and it's not empty
